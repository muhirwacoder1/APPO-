...
describe('AuthRouter', () => {
  ...
  // Login
  describe(`"POST:${Paths.Auth.Login}"`, async () => {
    ...
    it(`should return a response with a status of "${HttpStatusCodes.OK}" ` +
      'and a cookie with a jwt if the login was successful.', async done => {
        // Setup data
        const pwdHash = PwdUtil.hashSync(LoginCreds.password),
          ...
          loginUser = await User.User.create({
            name: 'john smith', email: LoginCreds.email,
            role: UserRoles.Standard, pwdHash
          });
        // Add spy
    ...
    it('should return a response with a status of ' +
      ...
      `"${Errors.Unauth}" if the password failed.`, async done => {
        // Setup data
